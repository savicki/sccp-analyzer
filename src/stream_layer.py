...
import re, datetime, time, sys, hashlib, binascii
#from enum import *
...
def __inspect_pkt(pkt, fdir, pkt_state):
    mask = PacketState.PartialEnd | PacketState.OneOfMultiple | PacketState.WithPriorInjects
...
def __print_sccp_packet(pkt, prefix = ''):
    if len(pkt[TCP].payload) > 0:
        ...
        if sccp.len + 8 != len(pkt[TCP].payload):
            ...
            print '%s[%s] : (msg_id: %s, msg_len: %s, msg_ver: %s) [%s bytes]' % (prefix, pkt.time, hex(sccp.msg), sccp.len + 8, sccp.res, len(pkt[TCP].payload))       
    # else:
...
def __inspect_msg(pkt, fdir, session_info, callback):
    ...
    ### print "[__inspect_msg] pkt_num: %s, tcp_len: %s, is skinny: %s" % (pkt_num, tcp_len, isinstance(tcp.payload, Skinny))
    ...
    if tcp_len == 0:
        ...
        while bytes_avail > 0:
            ...
            if dir_info.bytes_needed == 0: # e.g. '1' or '1/2 + 1'
                ...
                # don't trust @sccp.len as is - it may point to value somewhere in non-sccp-header
                sccp_len = sccp.len + 8

                ### print "[__inspect_msg] tcp_len: %s, sccp_len: %s" % (tcp_len, sccp_len)

                # e.g. 8x "\x00" bytes
                if sccp.len == 0:
                    
                    sys.stderr.write( "[__inspect_msg] bogus sccp_len: %s\n" % (sccp.len) )

                    action |= InspectAction.EmptyBytes
                    bytes_avail -= 4
                    dir_info.zero_bytes += 4

                    tcp_payload_bytes = __str_to_bytes(str(tcp.payload).encode("HEX"))
                    tcp.remove_payload()
                    ...
                    sys.stderr.write( "[__inspect_msg] [pkt.time: %s] [tcp.len: %s] unaligned sccp_len: %s, exit\n" % (
                ...
                if tcp_len == sccp_len: # e.g. ^1^ + '1'
                    ...
                    callback(pkt, fdir, PacketState.Single | (0 if len(inject_pkts) == 0 else PacketState.WithPriorInjects))

                # more data required - msg spread over pkt
                elif sccp_len > bytes_avail: # e.g. ^1^ + '1/2'

                    ### print "[__inspect_msg] sccp_len: %s > bytes_avail: %s" % (sccp_len, bytes_avail)

                    dir_info.cached_payload.append(str(tcp.payload).encode("HEX"))
                    tcp.remove_payload()
                    ...
                    #callback(pkt, fdir, PacketState.PartialBegin | 0 if len(inject_pkts) == 0 else PacketState.WithPriorInjects)

                # pkt contains 1+ msg(s)
                elif sccp_len < tcp_len: # e.g. ^1^ + '1+1/2'
                    #print '[%s] : tcp_len: %s, sccp_len: %s @ %s, csum: %s' % (fdir, tcp_len, sccp_len, pkt.time, hex(tcp.chksum))
                    ...
                    inject_pkt[TCP].do_dissect_payload(binascii.unhexlify(tcp_payload))
                    ...
                    callback(inject_pkt, fdir, PacketState.OneOfMultiple | (0 if inject_pkts_len == 0 else PacketState.WithPriorInjects))
                    #print inject_pkts_len, (PacketState.OneOfMultiple | (0 if inject_pkts_len == 0 else PacketState.WithPriorInjects))

            # dir_info.bytes_needed != 0
            else: 

                # there are enough bytes to complete cached msg
                if bytes_avail >= dir_info.bytes_needed:  # e.g. (1/2) + '1/2' , (1/2) + '1/2 + 1' , (1/2) + '1/2 + 1/2'
                    ...
                    needed_payload = __get_bytes(tcp_payload_bytes, 0, dir_info.bytes_needed)

                    if bytes_avail == dir_info.bytes_needed: # e.g. (1/2) + '1/2'
                        inject_pkt = pkt
                        action |= InspectAction.Pass
                    else: # e.g. (1/2) + '1/2 + 1'
                        ...
                        remain_payload = __get_bytes(tcp_payload_bytes, dir_info.bytes_needed)
                        tcp.do_dissect_payload(binascii.unhexlify(remain_payload))
                        tcp_len = len(tcp.payload)
                    ...
                    inject_pkt[TCP].do_dissect_payload(binascii.unhexlify(cached_payload + needed_payload))
                    dir_info.stats.msgs_injected += 1
                ...
                # msg continuation, add to cache and drop pkt - still need more data
                else: # e.g. ^1/3^ + '1/3'
                    dir_info.cached_payload.append(str(tcp.payload).encode("HEX"))
...
def get_msg_flow(pkt_flow):
    ...
    for pkt in pkt_flow:
        ...
        act, inject_pkts = __inspect_msg(pkt, get_dir(pkt.dport), msg_flow_acc, __inspect_pkt)
    ...
    if (stream_errors & InspectAction.EmptyBytes) == 0:
        if bytes_len_after != bytes_len_before:
            sys.stderr.write( "[get_msg_flow] bytes length mismatch: %s -> %s\n" % (bytes_len_before, bytes_len_after) )
        
        if str(orig_hash.digest()).encode("HEX") != str(mod_hash.digest()).encode("HEX"):
